...
tit1.markdown("<h1 style='text-align: center;'><u>HealthCare Using ML</u></h1>", unsafe_allow_html=True)
...
classifier_name = st.sidebar.selectbox("Select Classifier: ", ("Logistic Regression", "KNN", "SVM", "Decision Tree", "Random Forest", "Gradient Boosting", "XGBoost"))
...
def add_parameter_ui(clf_name):
	...
	if clf_name == "Logistic Regression":
		...
		kernel = st.sidebar.selectbox("Kernel", ("linear", "ploy", "rbf", "sigmoid", "precomputed"))
		params["C"] = C
		...
		L = st.sidebar.selectbox("Loss", ('deviance', 'exponentail'))
		M = st.sidebar.slider("max_depth", 2, 20)
		params["N"] = N
		params["LR"] = LR
		params["L"] = L
...
def get_classifier(clf_name, params):
	...
	if clf_name == "Logistic Regression":
		...
		clf = XGBClassifier(booster="gbtree",n_estimators=params["N"],max_depth=params["M"],learning_rate=params["LR"],objective=params["O"],gamma=params["G"],reg_alpha=params["A"],reg_lambda=params["L"],colsample_bytree=params["CS"])
...
def compute(Y_pred,Y_test):
	...
	st.text('Precision: {} \nRecall: {} \nF1-Score: {} \nAccuracy: {} %\nMean Squared Error: {}'.format(round(precision, 3), round(recall, 3), round(fscore,3), round((acc*100),3), round((mse),3)))
...
def user_inputs_ui(dataset_name, data):
	...
	if dataset_name == "Breast Cancer":
		...
		for col in X.columns:
			name=col
			col_val = st.number_input(col, abs(X[col].min()-round(X[col].std())), abs(X[col].max()+round(X[col].std())))
			user_val[name] = round((col_val), 4)
...
with st.expander("See more"):
    st.markdown("""In this section you can use your own values to predict the target variable. 
...
def user_predict():
	...
	if dataset_name == "Breast Cancer":
		...
		X = data.drop(["Outcome"], axis=1)
		U_pred = clf.predict([list(user_val[col] for col in X.columns)])
